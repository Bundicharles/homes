...
import { interestedAPI } from '@/services/api';
import { useSettings } from '@/context/SettingsContext';
import { useAuth } from '@/context/AuthContext';
import { ShoppingCart, Trash2, ExternalLink, AlertCircle } from 'lucide-react';
...
const MyInterested = () => {
  ...
  useEffect(() => {
    document.title = `My Interested Properties | ${businessName}`;
    const metaDesc = document.querySelector('meta[name="description"]');
    if (metaDesc) {
      metaDesc.setAttribute(
        'content',
        'View and manage properties you have expressed interest in. Remove properties from your interested list at any time.'
      );
    }
  }, [businessName]);
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-error mb-4">
          Failed to load interested properties. Please try again.
        </p>
        <button onClick={() => refetchInterested()} className="btn btn-primary">
          Retry
        </button>
      </div>
    );
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted mt-1">
            {interested.length}{' '}
            {interested.length === 1 ? 'property' : 'properties'} marked as
            interested
          </p>
        </div>
        <Link
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted mt-1">
                    Marked {getRelativeTime(item.created_at)}
                  </p>
                </div>
